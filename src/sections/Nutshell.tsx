...
const nutshellData = [
  {
    ...
    answer: "I break work into clear milestones with visible progress updates, using tools like Jira or Trello to track scope and flag risks early — so there are no last-minute surprises."
  },
  {
    question: "How do you handle challenges or roadblocks during a project?",
    answer: "I break the problem down systematically — a habit from my Mechanical Engineering background — isolate what's actually failing, and validate fixes with tests before moving on. If a blocker needs more context, I flag it early rather than letting it stall progress silently."
  },
  {
    question: "How do you ensure code quality?",
    answer: "I write in TypeScript wherever possible for type safety, follow component-driven architecture, and use AI-assisted code review as one layer of catching edge cases before shipping — on top of manual testing and review."
  },
  {
...
export const Nutshell = () => {
  ...
  const hotspots = [
    ...
    { id: 3, x: "42%", y: "62%", icon: "🤝", title: "🤝 Reliable & Collaborative — REST API integration, clear communication, milestone-driven delivery", detail: "REST APIs, bi-weekly sprints & daily standups" },
  ];
    ...
    <section className="text-white w-[95%] px-0 lg:max-w-5xl mx-auto py-16">
      ...
      >
        ...
        <SectionHeader
          ...
          description="Combining strong front-end engineering with a clear, collaborative process from kickoff to launch."
        />
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 relative z-10 w-full mt-4">
        ...
        >
          ...
          <div className="absolute bottom-6 left-6 right-6 z-20 pointer-events-none">
            ...
            <p className="text-white/70 text-sm sm:text-base max-w-md drop-shadow-md">A clear, structured workflow focused on quality and communication.</p>
          </div>
          ...
          {hotspots.map((spot) => (
            ...
            >
              <motion.button
                ...
                className={`relative flex h-8 w-8 items-center justify-center rounded-full backdrop-blur-md text-sm font-extrabold cursor-pointer shadow-xl transition-all duration-300 ${activeHotspot === spot.id
        ...
        >
          ...
          <div>
            ...
            <p className="text-white/60 text-sm leading-relaxed">Drop me a line. I usually respond within <span className="text-emerald-400 font-semibold">a day</span>.</p>
          </div>
            ...
            {nutshellData.map((item, index) => {
              ...
              return (
                <div key={index} className={`w-full rounded-2xl transition-all duration-300 ${isOpen ? 'bg-white/10' : 'bg-transparent hover:bg-white/5'}`}>
                  ...
                  >
                    <span className={`text-[15px] sm:text-base font-medium tracking-tight transition-colors duration-300 ${isOpen ? "text-emerald-400" : "text-white/80 group-hover:text-white"
                      }`}>
                      {item.question}
                    </span>

                    <span className={`shrink-0 transition-transform duration-500 ease-[0.32,0.72,0,1] ml-4 ${isOpen ? "-rotate-180 text-emerald-400" : "text-white/30 group-hover:text-white/50"
                      }`}>
